...
public:
  ...
  //function to transfer keys from HT to vector
  vector<string> getKeys(vector<string> mapKeys)
  {
      this->N = N;
      for (int i = 0; i < N; i++){
          if (this->HT[i].ContainsData == true){
              mapKeys.push_back(this->HT[i].Key);
          }
      }
      return mapKeys;
  }
  ...
  bool insert(KeyT key, ValueT value, function<int(KeyT,int)> Hash)
  {
    ...
    // valid array index, store into hash table:
